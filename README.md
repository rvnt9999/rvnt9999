...
| | SNS | OhShii |
|---|---|---|
| To start | An NNS neuron with at least 8 ICP staked, and an NNS proposal where the community votes on your tokenomics and can reject them | No proposal and no external vote on whether a project may proceed; eligibility is objective and enforced in code |
| Canisters deployed | Five core, deployed automatically by SNS-W (Governance, Root, Swap, Ledger, Index), plus one or more Archives as the ledger grows | One. A single SONS governance canister per DAO carries proposals, voting, treasury, locks and vesting; the token keeps its own standard ledger and index |
...
| Launch | Eleven stages, three of them manual, with a minimum participant count and an ICP floor that fail the swap and refund everyone if they are missed | An LGE, refunded automatically if it does not complete |
